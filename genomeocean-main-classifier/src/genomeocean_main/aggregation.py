"""Aggregate main-model predictions from chunks to contigs and files."""
...
PROBABILITY_COLUMNS = {
    0: "prob_cellular",
    1: "prob_ncldv_mirus",
    2: "prob_other_viruses",
}
COUNT_COLUMNS = {
    0: "n_chunks_cellular",
    1: "n_chunks_ncldv_mirus",
    2: "n_chunks_other_viruses",
}


def _winner(values: Iterable[int], labels: Mapping[int, str]) -> tuple[int, int]:
    """Return majority label and vote count; lower label ID wins an exact tie."""
...
def aggregate_chunks_to_contigs(chunk_results: pd.DataFrame) -> pd.DataFrame:
    """Create one result row per input contig."""
    ...
    for group_key, group in chunk_results.groupby(group_columns, sort=False, dropna=False):
        ...
        winner, votes = _winner(group["predicted_label"], MAIN_LABELS)
        ...
        row["predicted_name"] = MAIN_LABELS[winner]
...
def aggregate_contigs_to_files(contig_results: pd.DataFrame) -> pd.DataFrame:
    """Create one majority-vote result row per input FASTA file."""

    count_columns = {
        0: "n_contigs_cellular",
        1: "n_contigs_ncldv_mirus",
        2: "n_contigs_other_viruses",
    }
    output_columns = [
        "source_file",
        "n_contigs",
    ...
    for source_file, group in contig_results.groupby("source_file", sort=False):
        ...
        row = {
            ...
            "predicted_name": MAIN_LABELS[winner],
