"""Hugging Face model loading and end-to-end main classification."""
...
from .fasta import FastaRecord, read_input_records, write_fasta
...
class ModelSpec:
    """One ensemble member stored locally or on Hugging Face."""
...
def build_model_specs(
    ...
    *,
    ...
    """Normalize CLI/config model values into explicit ensemble members.

    There are two supported layouts:

    1. Repeat ``model_ids`` for independent local paths or Hugging Face repos.
    2. Give one ``model_id`` and several subfolders such as fold1..fold5.
    """
...
class ProbabilityEnsembleAccumulator:
    """Accumulate model probabilities without retaining every model output."""
...
class MainPredictor:
    """Predict with one model or a sequential soft-voting model ensemble."""
    ...
    def _validate_local_model_path(spec: ModelSpec) -> None:
        """Fail early with a useful message for raw DOEJGI checkpoints."""
    ...
    def _predict_one_model(
        ...
        for start in range(0, len(chunks), self.batch_size):
            ...
            if probabilities.shape[1] != len(MAIN_LABELS):
                raise RuntimeError(
                    f"Main model '{spec.name}' returned {probabilities.shape[1]} labels; "
                    f"expected {len(MAIN_LABELS)}"
                )
    ...
    def predict_chunks(self, chunks: Sequence[SequenceChunk]) -> pd.DataFrame:
        """Average fold probabilities and return one ensemble row per chunk."""

        if not chunks:
            return pd.DataFrame(columns=MAIN_CHUNK_RESULT_COLUMNS)

        accumulator = ProbabilityEnsembleAccumulator(
            n_items=len(chunks),
            n_labels=len(MAIN_LABELS),
        )
        for spec in self.model_specs:
            try:
                ...
                # _predict_one_model owns only one model at a time. Once it
                # returns (or raises), clearing the allocator lets the next
                # fold reuse the same GPU memory.
        ...
        for index, chunk in enumerate(chunks):
            ...
            row["predicted_name"] = MAIN_LABELS[label]
            ...
            rows.append(row)
        return pd.DataFrame(rows, columns=MAIN_CHUNK_RESULT_COLUMNS)
...
def write_main_outputs(
    bundle: MainPredictionBundle,
    output_dir: str | Path,
    *,
    metadata: dict | None = None,
) -> dict[str, Path]:
    """Write stable, user-facing result files."""
    ...
    paths = {
        ...
        "candidates": out / "ncldv_mirus_candidates.fna",
        "candidate_manifest": out / "ncldv_mirus_candidate_manifest.tsv",
        "metadata": out / "run_metadata.json",
    }
    bundle.chunk_results.reindex(columns=MAIN_CHUNK_RESULT_COLUMNS).to_csv(
    ...
    candidate_ids = set(
        bundle.contig_results.loc[
            bundle.contig_results["predicted_label"] == 1,
            "record_id",
        ]
    )
    candidates = [record for record in bundle.records if record.record_id in candidate_ids]
    write_fasta(candidates, paths["candidates"], use_record_id=True)
    pd.DataFrame(
        [
            {
        ...
        columns=["candidate_record_id", "source_file", "contig_id"],
    ).to_csv(paths["candidate_manifest"], sep="\t", index=False)
    ...
    run_metadata = {
        **(metadata or {}),
        ...
        "ncldv_mirus_candidates": len(candidates),
